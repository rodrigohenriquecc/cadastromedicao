...
export function LeftSidebarPanel({
  ...
  return (
    <div className="pointer-events-auto fixed left-0 top-0 bottom-0 z-[1000] select-none font-sans">
      ...
      <div
        className={`relative flex h-full w-[340px] md:w-[380px] flex-col border-r border-[#212529] bg-[#343a40] text-slate-100 shadow-2xl transition-transform duration-300 ease-in-out ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* Top Header Banner (Authentic DER Blue Navbar) */}
        <div className="flex shrink-0 items-center justify-between bg-[#207ba1] px-3.5 py-3 text-white shadow-md border-b border-[#185e7b]">
          ...
          {hasAnyFilter && (
            <button
              type="button"
              onClick={onResetAll}
              className="flex items-center gap-1 rounded border border-[#ffc107] bg-[#ffc107]/10 px-2 py-1 text-[11px] font-bold text-[#ffc107] hover:bg-[#ffc107] hover:text-slate-950 transition shadow-sm"
        ...
        <div className="shrink-0 bg-[#2b3035] p-2.5 border-b border-[#212529]">
          <div className="relative flex items-center">
            <input
              ...
              className="w-full rounded bg-[#23272b] border border-[#495057] py-1.5 pl-3 pr-8 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-[#207ba1] transition"
            />
        ...
        <div className="shrink-0 bg-[#272b30] p-3 border-b border-[#212529]">
          <div className="flex items-center justify-between text-[11px] text-slate-300 font-semibold mb-1.5">
            ...
            <span className="rounded bg-[#207ba1] px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
              {visibleCount} de {totalCount} serv.
            </span>
          </div>
          <div className="flex items-baseline justify-between rounded bg-[#1c2024] border-l-4 border-l-[#ffc107] border border-[#40464d] px-3 py-2 text-[#ffc107] shadow-inner">
            <span className="text-[11px] font-black uppercase tracking-wider text-slate-300">QNTD ACUMULADA</span>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#212529] bg-[#2f353a]">
          ...
          <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-3.5 pt-3 pb-1">
          ...
          <div>
            <button
              ...
              className="flex w-full items-center justify-between bg-[#24282c] px-4 py-2 text-left text-xs font-bold text-slate-200 hover:bg-[#207ba1] hover:text-white transition cursor-pointer"
            >
              <span className="flex items-center gap-2">
                <FileSpreadsheet size={15} className="text-amber-400" />
                1. Importação (CM.xlsx)
              </span>
              {sections.upload ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
            </button>
            ...
            {sections.upload && (
              <div className="p-3 bg-white text-slate-900 border-t border-slate-300">
                <div
                  ...
                  className={`cursor-pointer rounded-lg border-2 border-dashed p-3 text-center transition ${
                    dragging
          ...
          <div>
            ...
            >
              <span className="flex items-center gap-2">
                <ListFilter size={15} className="text-teal-400" />
                2. Registro RC ({rcs.length})
              </span>
              <div className="flex items-center gap-2">
                {selectedRcs.length > 0 && (
                  <span className="rounded bg-teal-600 px-2 py-0.5 text-[10px] font-bold text-white">
                    {selectedRcs.length} sel.
                  </span>
                )}
                {sections.rcs ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
              </div>
            </button>
              ...
              <div className="p-3 bg-white text-slate-900 border-t border-slate-300">
                <Select
                  ...
                  styles={{
                    ...
                    menu: (base) => ({ ...base, fontSize: 12, zIndex: 2000, background: "#ffffff" }),
                    multiValue: (base) => ({
                      ...base,
          ...
          <div>
            ...
            >
              <span className="flex items-center gap-2">
                <Route size={15} className="text-[#ffc107]" />
              ...
              <div className="flex items-center gap-2">
                ...
                {sections.highways ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
              </div>
            </button>

            {sections.highways && (
              <div className="p-3 space-y-2.5 bg-white text-slate-900 border-t border-slate-300">
                <Select
                  isMulti
                  isClearable
                ...
                <div className="flex items-center justify-between gap-2">
                  <button
                    ...
                    className="flex-1 rounded bg-[#207ba1] py-1.5 text-[11px] font-bold text-white hover:bg-[#1a6291] transition shadow-sm cursor-pointer text-center"
                  >
                    Selecionar Todas
                  </button>
                  {selectedHighways.length > 0 && (
                    <button
                      ...
                      className="rounded border border-[#dc3545] bg-[#dc3545]/10 px-3 py-1.5 text-[11px] font-bold text-[#dc3545] hover:bg-[#dc3545] hover:text-white transition shadow-sm cursor-pointer"
                    >
                      Limpar
                    </button>
                  )}
                ...
                {selectedHighways.length > 0 && (
                  <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-slate-300 pt-1">
                    {selectedHighways.map((sp) => {
                      const h = activeHighways.find((item) => item.sp === sp);
                      return (
                        <div
                          key={sp}
                          className="flex items-center justify-between rounded p-2 bg-slate-50 border border-slate-300 shadow-sm text-xs font-semibold text-slate-900"
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="flex h-5 items-center justify-center rounded bg-[#207ba1] px-2 font-mono text-[11px] font-bold text-white uppercase">
                              {sp}
                            </span>
                          ...
                          <button
                            ...
                            className="flex h-6 w-6 items-center justify-center rounded bg-sky-100 text-[#207ba1] hover:bg-[#207ba1] hover:text-white transition border border-sky-300 cursor-pointer shrink-0"
                            title={`Zoom em ${sp}`}
                          >
                            <Crosshair size={12} />
                          </button>
          ...
          <div>
            ...
            >
              <span className="flex items-center gap-2">
                <ListFilter size={15} className="text-sky-300" />
              ...
              <div className="flex items-center gap-2">
                ...
                {sections.descriptions ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
              </div>
            </button>
              ...
              <div className="p-3 bg-white text-slate-900 border-t border-slate-300">
                ...
                <div className="mt-3 rounded-lg border border-slate-300 bg-white p-2.5 shadow-sm">
                  ...
                  {(selectedHighways.length > 0 || highwaySearch.trim() || selectedRcs.length > 0 || selectedDescriptions.length > 0) && (
                    <div className="flex flex-wrap gap-1 mb-2">
                      {selectedHighways.map((hw) => (
                        <span key={hw} className="inline-flex items-center gap-1 rounded bg-sky-100 border border-sky-300 px-1.5 py-0.5 text-[9px] font-bold text-sky-900">
                          {hw}
                        </span>
                      ))}
                      {highwaySearch.trim() && (
                      ...
                      {selectedRcs.length > 0 && (
                        <span className="inline-flex items-center gap-1 rounded bg-teal-100 border border-teal-300 px-1.5 py-0.5 text-[9px] font-bold text-teal-900">
                          {selectedRcs.length} RC(s)
                        </span>
                      )}
                      {selectedDescriptions.length > 0 && (
                        <span className="inline-flex items-center gap-1 rounded bg-indigo-100 border border-indigo-300 px-1.5 py-0.5 text-[9px] font-bold text-indigo-900">
                  ...
                  {filteredVisibleServices.length === 0 ? (
                    <div className="py-3 text-center text-[11px] font-semibold text-slate-500 bg-slate-50 rounded border border-slate-200">
                      Nenhum serviço filtrado para as seleções atuais.
                    </div>
                  ) : (
                    <div className="max-h-64 overflow-x-auto overflow-y-auto rounded border border-slate-300 scrollbar-thin scrollbar-thumb-slate-400">
                      <table className="w-full min-w-[420px] border-collapse text-left text-[10px] font-semibold">
                        <thead className="sticky top-0 bg-[#207ba1] text-white font-bold uppercase tracking-wider text-[9px] shadow-sm z-10">
                          <tr>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">KM INICIAL</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">SP</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">RC</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">SENTIDO</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">COMPR.</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">LARG.</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b]">ALT./ESP.</th>
                            <th className="px-1.5 py-1 border-b border-[#185e7b] text-right">QNTD</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-200 bg-white text-slate-900">
                          {filteredVisibleServices.map((pt, idx) => (
                            <tr key={pt.id || idx} className="hover:bg-sky-50/70 transition">
                              <td className="px-1.5 py-1 font-mono font-bold text-slate-900 whitespace-nowrap">
                                {pt.kmInicial.toFixed(3).replace(".", ",")}
                              </td>
                              <td className="px-1.5 py-1 font-mono font-bold text-slate-800 whitespace-nowrap">
                                {pt.sp}
                              </td>
                              <td className="px-1.5 py-1 font-mono font-bold text-teal-900 whitespace-nowrap">
                                {pt.rc || "-"}
                              </td>
                              <td className="px-1.5 py-1 text-slate-700 whitespace-nowrap">
                                {pt.sentido || "-"}
                              </td>
                              <td className="px-1.5 py-1 font-mono text-[#207ba1] whitespace-nowrap">
                                {formatBrTwoDecimals(pt.comprimento)}
                              </td>
                              <td className="px-1.5 py-1 font-mono text-slate-700 whitespace-nowrap">
                                {formatBrTwoDecimals(pt.largura)}
                              </td>
                              <td className="px-1.5 py-1 font-mono text-slate-700 whitespace-nowrap">
                                {formatBrTwoDecimals(pt.altEsp)}
                              </td>
                              <td className="px-1.5 py-1 font-mono font-extrabold text-[#207ba1] text-right whitespace-nowrap">
                        ...
                        {totalQuantidade !== null && (
                          <tfoot className="sticky bottom-0 bg-[#175773] text-white font-bold uppercase text-[10px] shadow-lg z-10">
                            <tr>
                              <td colSpan={7} className="px-2 py-1.5 text-right font-bold border-t border-[#103d52] tracking-wider text-slate-200">
                                TOTAL QNTD:
                              </td>
                              <td className="px-1.5 py-1.5 text-right font-mono font-black text-[#ffc107] text-xs border-t border-[#103d52] whitespace-nowrap">
                                {totalQuantidade.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </td>
                            </tr>
                          </tfoot>
        ...
        {/* Sidebar Toggle Handle Button (Attached to sidebar right edge) */}
        <button
          type="button"
          onClick={() => setOpen((prev) => !prev)}
          className="absolute left-full top-1/2 -translate-y-1/2 flex h-16 w-8 items-center justify-center rounded-r-lg border-2 border-l-0 border-[#212529] bg-[#207ba1] text-white shadow-2xl hover:bg-[#1a6291] transition active:scale-95 cursor-pointer z-50"
